-- FIXED F1TENTH Cartographer Configuration for 2D SLAM
-- Save this as: config/f1tenth_2d.lua
...
options = {
  ...
  published_frame = "odom",  -- CHANGED: Use odom frame to avoid conflicts
  odom_frame = "odom",
  provide_odom_frame = false,  -- CHANGED: Don't provide odom (you have vesc_to_odom_node)
  ...
  -- CRITICAL: Reduce sampling ratios to avoid timestamp conflicts
  rangefinder_sampling_ratio = 1.,
  odometry_sampling_ratio = 0.1,  -- CHANGED: Sample every 10th odom message
...
-- Trajectory Builder 2D Configuration for F1TENTH (FIXED)
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 35
TRAJECTORY_BUILDER_2D.min_range = 0.1
TRAJECTORY_BUILDER_2D.max_range = 10.0
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 1.0

-- CRITICAL: Completely disable IMU usage
TRAJECTORY_BUILDER_2D.use_imu_data = false
TRAJECTORY_BUILDER_2D.imu_gravity_time_constant = 10.

-- CRITICAL: Enable online correlative scan matching for odometry-only SLAM
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = true 
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.15  -- INCREASED
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.angular_search_window = math.rad(20.)  -- INCREASED
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 10.
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.rotation_delta_cost_weight = 1e-1

-- Ceres Scan Matcher (Fine-tuned for F1TENTH)
...
-- Motion Filter (Tuned for F1TENTH dynamics)
...
-- Loop Closer (For better map consistency)
